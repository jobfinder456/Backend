...
// Utility function to fetch Razorpay subscription details
async function fetchRazorpaySubscription(subscriptionId) {
  try {
    ...
    throw new Error("Failed to fetch Razorpay subscription details");
  }
}
...
router.post("/verify-subscription", authMiddleware, async (req, res) => {
  ...
  try {
    const subscriptionDetails = await fetchRazorpaySubscription(subscriptionId);
...
router.post("/cancel-subscription", authMiddleware, async (req, res) => {
  ...
  try {
    const subscriptionDetails = await fetchRazorpaySubscription(subscription_id);
    const expiryTimestamp = subscriptionDetails.current_end;
...
router.get("/get-subscription", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const { plan_id, customer_id, current_start, current_end, charge_at, end_at } = subscriptionDetails;
    const planNames = {
